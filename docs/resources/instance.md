...
- **Force replacement:** `cloud_provider`, `region`, `template` (an OS/template
  ...
  `ssh_key`, `network`, `network_plan`, `assign_public_ip`, `storage_category`).

**Networking.** Attach the instance to a network you manage with `network` (the
slug of a `zcp_network`), or let the platform auto-create one with
`network_plan`. They are mutually exclusive. Prefer `network`. `network_plan`
...
import ID (slash-separated, trailing/empty segments allowed):
...
- `network` (String) Slug of an existing `zcp_network` to attach the instance
  to. Mutually exclusive with `network_plan`. Preferred: you manage the network,
  so `terraform destroy` leaves nothing behind. Changing this forces
  replacement.
- `network_plan` (String) Network plan slug (e.g. `pnet-yow`) used to
  auto-create an isolated network. Mutually exclusive with `network`. Terraform
  does not manage the auto-created network and destroy does not remove it. Run
...
- `assign_public_ip` (Boolean) Whether to assign a public IP. Defaults to
  ...
  releases the auto-assigned IP (see the note above). Changing this forces
  replacement.
