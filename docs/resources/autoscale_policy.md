...
Manages a scale-up policy on a `zcp_autoscale_group`. All rule fields update in place; changing `autoscale_group` forces replacement. For scale-down rules use `zcp_autoscale_condition`.
