...
Registers an ISO image from a URL. The permission flags (`password_enabled`, `is_extractable`, `is_bootable`) update in place; every other change forces replacement.
...
- `url` (String) HTTP(S) URL the platform downloads the ISO from. Changing this forces replacement.
...
- `operating_system` (String) Operating system name (e.g. `Ubuntu`). Changing this forces replacement.
- `operating_system_version` (String) Operating system version (e.g. `24.04`). Changing this forces replacement.
- `password_enabled` (Boolean) Whether password reset is supported. Updated in place.
- `is_extractable` (Boolean) Whether the ISO is downloadable by other users. Updated in place.
- `is_bootable` (Boolean) Whether the ISO is bootable. Defaults to `true`. Updated in place.
