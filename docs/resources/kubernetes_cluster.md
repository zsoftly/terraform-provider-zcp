...
Changing `workers` scales the cluster **in place** (an update, not a replacement) and blocks until scaling completes. Changing `version` upgrades the cluster in place, and changing `plan` moves the nodes to the new plan in place. Changing `region` forces a new cluster.

`name`, `cloud_provider`, `region`, `billing_cycle`, `storage_category`, `project`, `ssh_key`, `control_nodes`, and `ha` are immutable. Changing any of them forces replacement.
...
- `version` (String) Kubernetes version slug (e.g. `v1.36.1`). Changing this upgrades the cluster in place.
- `plan` (String) Cluster node plan slug (e.g. `k8s-li-yow-1`). Changing this upgrades the cluster in place.
